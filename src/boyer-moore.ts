/**
 * Returns the index of the first occurence of pattern in text
...
/**
 * Create jump table based on mismatched character information
 *
 */
...
function makeOffsetTable(pattern: string): number[] {
  ...
  for (let i = pattern.length; i > 0; i--) {
    ...
    table[pattern.length - 1] = lastPrefixPosition - 1 + pattern.length;
  }
